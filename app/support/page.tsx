...
const META: Bi<{ title: string; description: string }> = {
  ...
  en: {
    ...
    description:
      'Contact channels, what to attach to a request and when we reply. What to do if the widget opens empty or amoCRM access has been revoked.',
  },
};
...
const CHANNEL_TEXT = {
  ...
  whatsappHint: {
    ru: 'Если Telegram у вас не в ходу. Отвечаем в тот же диалог.',
    en: 'If you do not use Telegram. We reply in the same chat.',
  },
  mailHint: {
    ru: 'Сюда же — вопросы по счетам, ключу лицензии и закрывающим.',
...
const ATTACH: readonly { title: Bi; text: Bi }[] = [
  ...
  },
  {
    ...
    text: {
      ru: 'Воронка, Менеджеры, Качество данных, Путь заявки — на какой вкладке видна проблема.',
      en: 'Funnel, Managers, Data quality, Lead path — which tab shows the problem.',
    },
  },
  {
...
const T = {
  h1: { ru: 'Поддержка', en: 'Support' },
  lead: {
    ru: 'Пишите напрямую тем, кто пишет код. Промежуточной линии, тикет-системы и робота-приёмщика у нас нет: команда маленькая, обращение читает инженер.',
    en: 'Write directly to the people who write the code. There is no first line, ticket system or intake bot: the team is small and an engineer reads your request.',
  },
  widget: { ru: 'виджет', en: 'widget' },
  hours: {
    ru: 'Отвечаем в рабочие часы по будням. Ночью и в выходные ответа может не быть — круглосуточной линии у нас нет.',
    en: 'We reply during business hours on weekdays. There may be no reply at night or at weekends — we have no 24/7 line.',
  },
  whereH2: { ru: 'Куда писать', en: 'Where to write' },
  introMany: {
    ru: 'Куда бы вы ни написали, обращение попадает к одним и тем же людям. Ссылки открывают готовый шаблон — его достаточно дозаполнить.',
    en: 'Whichever channel you choose, the request reaches the same people. The links open a ready-made template — just fill in the blanks.',
  },
  introOne: {
    ru: 'Обращение читает инженер, который писал этот расчёт. Ссылка открывает готовый шаблон — его достаточно дозаполнить.',
    en: 'Your request is read by the engineer who wrote the calculation. The link opens a ready-made template — just fill in the blanks.',
  },
  write: { ru: 'Написать', en: 'Write' },
  attachH2: { ru: 'Что приложить к обращению', en: 'What to attach to a request' },
  attachP: {
    ru: 'Мы не храним ни имён, ни телефонов, ни текстов, поэтому не можем найти вашу сделку по фамилии клиента и не видим, что у вас на экране. Разбор идёт по этим пяти пунктам.',
    en: 'We store no names, phone numbers or text, so we cannot find your deal by a client’s surname and cannot see your screen. The review runs on these five items.',
  },
  whenH2: { ru: 'Когда мы отвечаем', en: 'When we reply' },
  whenP1: {
    ru: 'Заявленного времени реакции в часах у нас нет, и выдумывать его мы не будем. Отвечаем в рабочие часы по будням, обычно в тот же рабочий день. Если вопрос требует разбора данных вашего аккаунта, сначала приходит подтверждение, что обращение взяли, потом разбор.',
    en: 'We have no declared response time in hours and will not invent one. We reply during business hours on weekdays, usually the same business day. If the question needs a review of your account data, you first get a confirmation that the request is taken, then the review.',
  },
  whenP2: {
  ...
  emptyP: {
    ru: 'Три причины по убыванию частоты. Первые две решаются без нас, третья — одним письмом.',
    en: 'Three causes, most frequent first. The first two you can fix without us; the third takes one message.',
  },
  cacheH3: { ru: 'Закэшировался старый код', en: 'Old code is cached' },
  cacheP: {
    ru: 'amoCRM кэширует файлы виджета. Обновите страницу с очисткой кэша: Cmd+Shift+R на macOS, Ctrl+F5 на Windows. Это лечит большинство пустых экранов.',
    en: 'amoCRM caches widget files. Reload the page bypassing the cache: Cmd+Shift+R on macOS, Ctrl+F5 on Windows. This fixes most empty screens.',
  },
  ...
  revokedP: {
    ru: 'Администратор мог отозвать доступ в разделе «Выданные доступы», или он слетел при смене секретного ключа интеграции. Лечится повторным подключением по OAuth — виджет попросит об этом сам. История не теряется.',
    en: 'An administrator may have revoked access under “Granted access”, or it was lost when the integration secret was rotated. The fix is to reconnect via OAuth — the widget will ask for it. No history is lost.',
  },
  ...
  methodP: {
    ru: 'Если цифра в отчёте не сходится с ожиданием, ответ чаще всего здесь: медиана вместо среднего, порог по числу сделок, отказ строить разрез на пустом поле.',
    en: 'If a number in a report does not match your expectation, the answer is usually here: median instead of mean, a threshold on deal count, refusing to build a breakdown on an empty field.',
  },
  open: { ru: 'Открыть', en: 'Open' },
  bugH2: { ru: 'Нашли ошибку в цифрах', en: 'Found an error in the numbers' },
  bugP1: {
    ru: 'Это самое ценное обращение, и оно идёт вне очереди. Пришлите поддомен, вкладку, период и число, которое считаете неверным, — мы поднимем расчёт по вашим данным и ответим, ошибка это или разница в определении метрики. Если ошибка наша, скажем прямо и напишем, что починили.',
    en: 'This is the most valuable request and it skips the queue. Send the subdomain, tab, period and the number you believe is wrong — we will rerun the calculation on your data and tell you whether it is an error or a difference in metric definition. If the error is ours, we will say so and report what we fixed.',
  },
  bugP2: {
